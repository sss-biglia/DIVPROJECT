...
export function MPPaymentSheet({
  ...
  const openMercadoPago = async (alias?: string, paymentLink?: string) => {
    ...
    if (alias) {
      ...
      setCopiedMessage('Alias copied! Opening MercadoPago...');
    ...
    <Modal transparent visible={visible} animationType="none" onRequestClose={onClose}>
      <Animated.View style={[styles.backdrop, { opacity: backdropOpacity }]}>
        ...
        >
          ...
          <Text style={styles.title}>Pay {memberName}</Text>
          ...
          {mpPaymentLink || mpAlias ? (
            ...
            <>
              <Text style={styles.muted}>Member hasn't set up MercadoPago</Text>
              <PrimaryButton
                label="Open MercadoPago"
                onPress={() => void openMercadoPago(mpAlias, mpPaymentLink)}
                variant="neutral"
              />
            </>
          ...
          <GhostButton
            label="Mark as settled manually ✓"
            onPress={() => {
              onSettle();
              onClose();
            }}
